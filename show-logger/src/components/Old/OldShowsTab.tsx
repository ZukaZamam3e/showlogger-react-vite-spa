...
export const ShowsTab = (props: ShowsTabProps) => {
  ...
  const fetchData = () => {
    setLoaded(false);
    apiShowLoad
      .execute('GET', protectedResources.oaprojectsApi.showEndpoint + '/load')
      .then((res: any) => {
        if (!!res) {
          console.log(res);
  ...
  const fetchMoreData = (page: number) => {
    ...
    apiShowLoad
      .execute(
        'GET',
        protectedResources.oaprojectsApi.showEndpoint +
          `/get?offset=${offset}&take=${take}&search=${search}`,
      )
      .then((res: any) => {
        if (!!res) {
          console.log(res);
  ...
  const onDeleteShow = (showId: number) => {
    apiShow
      .execute(
        'POST',
        protectedResources.oaprojectsApi.showEndpoint + '/Delete',
        { showId: showId },
      )
      .then((res: any) => {
        if (!!res.model) {
