import { Box, Button, Card, Fab, FormControl, FormControlLabel, FormGroup, InputAdornment, InputBaseComponentProps, InputLabel, OutlinedInput, Paper, Stack, Switch, TextField, ToggleButton, ToggleButtonGroup } from "@mui/material";
...
export const EditShow = (props: EditShowProps) => {
    ...
    // const [amcTax, setAmcTax] = useState(0);
    // const [amcRewards, setAmcRewards] = useState(0);
    // const [amcBenefits, setAmcBenefits] = useState(0);
    // const [amcAlistTicket, setAmcAlistTicket] = useState(0);
    // const [amcTicket, setAmcTicket] = useState(0);
    // const [amcTicketQauntity, setAmcTicketQauntity] = useState(0);
    const [newTransactionId, setNewTransactionId] = useState(-1);

    const getAmcInfoValue = (id: number, quantity: boolean = false) => {
        const transaction = show.transactions?.filter(m => m.transactionTypeId == id);
    ...
    const sumPurchases = !!purchases ? purchases.reduce((sum, current) => sum + (current.deleteTransaction ? 0 : current.costAmt), 0) : 0;

    const purchaseTotal = (sumPurchases + amcTax - amcRewards - amcBenefits).toFixed(2);
    ...
    const handleTransactionItemChange = (id: number, e: any) => {
        if (!!show.transactions) {
            const updatedTransactions = show.transactions.map((transaction: TransactionModel) => {
    ...
    const amcInfoUpdate = (name: string, amount: string) => {
        ...
        let amcInfo = transactions?.filter(item => item.transactionTypeId == transactionTypeId);
        ...
        const updatedTransactions = transactions.map((transaction: TransactionModel) => {
