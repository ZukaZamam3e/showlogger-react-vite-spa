...
export const MoviesTab = () => {
  const { getMovies, loadMovies, saveWatched, deleteWatched, createWatched } =
    watchedApi();
  ...
  const load = async () => {
    const { data, count } = await loadMovies(take);
    setWatched(data);
    setWatchedCount(count);
  };

  const get = async (page: number, search: string) => {
    const { data, count } = await getMovies(page, take, search);
    setWatched(data);
    setWatchedCount(count);
  };
